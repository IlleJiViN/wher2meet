...
export default function Home() {
  ...
  const [routingMode, setRoutingMode] = useState('distance');
  ...
  return (
    <main className="min-h-screen max-w-md mx-auto bg-[#f2f4f6] pb-32 relative overflow-x-hidden">
        ...
        <div className="toss-card space-y-5">
          ...
          <div className="pt-2 border-t border-gray-100">
            <div className="flex justify-between items-center mb-2">
              <span className="text-[14px] font-bold text-[#505967]">중간 지점 계산 방식</span>
            </div>
            <div className="flex p-1 bg-[#f2f4f6] rounded-xl relative">
              <button
                onClick={() => setRoutingMode('distance')}
                className={`flex-1 py-2 text-[14px] font-bold rounded-lg transition-all ${routingMode === 'distance' ? 'bg-white text-[#191f28] shadow-sm' : 'text-[#8b95a1]'}`}
              >
                📍 직선 거리
              </button>
              <button
                onClick={() => setRoutingMode('time')}
                className={`flex-1 py-2 text-[14px] font-bold rounded-lg transition-all ${routingMode === 'time' ? 'bg-white text-[#191f28] shadow-sm' : 'text-[#8b95a1]'}`}
              >
                ⏱️ 소요 시간 (차량)
              </button>
            </div>
      ...
      {/* We removed the inline results to use the Bottom Sheet instead */}
      ...
      {/* Overlay component that slides up over the map */}
      {results && (
        <>
          {/* Backdrop */}
          <div 
